...
def regenerate_rsa_pair_key(conn, cipher, user):
    global signer, archive, CLIENT_PRIVATE_KEY, cipher_client_public, cipher_client
    change_key_req = {
    ...
    for key, value in archive.items():
        data[key] = []
        for cipher_text in value:
            decrypted_data = cipher_client.decrypt(cipher_text).decode()
    ...
    archive = {key: [cipher_client_public.encrypt(json.dumps(json_data).encode()) for json_data in value] for key, value in data.items()}
    time.sleep(0.2)
        ...
        while True:
            try:
                ...
                if command == "exit":
                    ...
                    print("Commands:\n--online-users\n--history-chat\n--register\n--login\n--logout\n--send_to_offline (sending message to offline user)\n--connect (set DH key with a user)\n--end2end\n--create_group\n--send_group_message\n--add_group_member\n--add_group_admin\n--delete_group_member")
                    continue
                elif command == "online-users":
                    message = user_name
                    data = {'message': message, 'command': command} # just must be not None
                    ...
                    regenerate_rsa_pair_key(client_socket, cipher_server, user_name)
                    continue
                    ...
                    receiver = input()
                    if receiver not in tcp_seq_num:
                        print("Set DH with receiver process:")
                        data = connect(receiver)
                        secure_send_message(client_socket, cipher_server, data)
                        print("Connection process completed. try again end2end message.")
                        continue
                    data = {
                        'command': command,
        ...
        while True:
            try:
                ...
                if hmac.compare_digest(received_hmac, hmac_digest):
                    if received_type == "success" or received_type == "fail":
                        ...
                        if is_valid:
                            ...
                            you_can_show_it_if_you_want = 1
                        else:
                            print("Signature is invalid. delete it.")
                        ...
                        dh_public_key = my_dh_private_key.public_key()
                        groups[data['group_name']] = {"parameters": parameters, "prv_dh_key": my_dh_private_key, "pub_dh_key": dh_public_key, "messages": []}
                        print(f"{data['adder']} added you to the group: {data['group_name']}")
                        ...
                        show_it_if_you_want_to_see_detail = 1
                        ...
                        # print("Invalid message from server", data)
                        show_it_if_you_want_to_see_detail = 1
                else:
